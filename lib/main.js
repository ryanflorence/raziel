...
commands.rm = function () {
  key = program.args[1];
  if (!key) return callback('# no password key to remove provided');
  var password = util.decrypt(data[key], master);
  delete data[key];
  callback('# ' + key + ' deleted, was: ' + password);
};
...
function install () {
  ...
  program.prompt("==> Enter your master password: ", function (input) {
    var hash = util.encryptHash( input.trim() );
    data = { master: hash };
    run();
  });
}
...
function authorize () {
  program.password('password?', function (input) {
    master = input;
    authorized = util.compareHash( input.trim(), data.master );
